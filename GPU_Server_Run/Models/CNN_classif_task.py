...
# -------------INCIPIT------------------
# Here the task is classification therefore assessing the reliability of the CNN in doing Sentiment Analysis. There are no predictions on the sentences webscraped that will be used to predict the stock prices.
...
def left_pad_sequences_with_torch(sequences, embedding_dim):
    padded_sequences = []
    for sequence in sequences:
        pad_length = embedding_dim - len(sequence)
...
tokenized_sentences = [word_tokenize(sentence) for sentence in df['sentence']]
...
vocab_df.to_csv(parent_directory + '/Output/CNN/vocabulary_with_indices_new_classif.csv')
...
left_padded_sequences_X_train = left_pad_sequences_with_torch(embedded_sequences_X_train, embedding_dim)
left_padded_sequences_X_val = left_pad_sequences_with_torch(embedded_sequences_X_val, embedding_dim)
left_padded_sequences_X_test = left_pad_sequences_with_torch(embedded_sequences_X_test, embedding_dim)
...
early_stopping = EarlyStopping(monitor='val_acc', patience=8, restore_best_weights=True) # if the validation loss does not improve for five consecutive epochs, early stopping will be triggered, and the training process will stop.
...
# Convert one-hot encoded Y_test labels back to original class labels
...
df_ET.to_csv(parent_directory + '/Output/CNN/execution_times_classif_pretrained.csv', index=False)


# Calculate the F1 score
f1 = f1_score(Y_test, Y_pred, average='weighted')  # You can specify the averaging method
precision = precision_score(Y_test, Y_pred, average='weighted')  # You can specify the averaging method
recall = recall_score(Y_test, Y_pred, average='weighted')  # You can specify the averaging method
...
df_metrics.set_axis(performance_metrics.keys())


# Save the DataFrame to a CSV file
df_metrics.to_csv(parent_directory + '/Output/CNN/cnn_performance_metrics_classif_wv_pretrained.csv', index=False)
...
plt.savefig(parent_directory + '/Output/CNN/accuracy_plot_classif_wv_pretrained.png')
plt.show()
...
plt.ylabel('Loss')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig(parent_directory + '/Output/CNN/loss_plot_classif_wv_pretrained.png')
plt.show()
