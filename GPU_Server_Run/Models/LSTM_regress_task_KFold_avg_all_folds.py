...
# Specifically this code will consider a K-Fold dataset splitting with the average results among all the K folds
# ---------------------------------------
...
data = pd.read_csv(parent_directory + '/Input_Data/combined_dataset_4_regression_with_CNN.csv', index_col = 0).drop(columns=['label_CNN','label_finbert','label_distilbert','Date'])
data_distilbert = pd.read_csv(parent_directory + '/Input_Data/combined_dataset_4_regression_with_CNN.csv', index_col = 0).drop(columns=['label_CNN','label_finbert','Date'])
data_finbert = pd.read_csv(parent_directory + '/Input_Data/combined_dataset_4_regression_with_CNN.csv', index_col = 0).drop(columns=['label_CNN','label_distilbert','Date'])
data_CNN = pd.read_csv(parent_directory + '/Input_Data/combined_dataset_4_regression_with_CNN.csv', index_col = 0).drop(columns=['label_finbert','label_distilbert','Date'])
...
# Create lists to store performance metrics and true/predicted prices
# mse_values = []
true_prices = []
all_mse_values_no_SA = []
all_mse_values_finbert = []
all_mse_values_distilbert = []
all_mse_values_CNN = []
predicted_prices_no_SA = []
predicted_prices_finbert = []
predicted_prices_distilbert = []
predicted_prices_CNN = []
ret_true = []
ret_no_SA = []
ret_finbert = []
ret_distilbert = []
...
for fold, (train_indices, test_indices) in enumerate(kf.split(scaled_features_no_SA)): # tanto sono lunghi uguali
    ...
    all_mse_values_no_SA.append(history_no_SA.history['loss'])  # Append MSE values for the current fold
    
    true_prices.append(y_test)
    predicted_prices_no_SA.append(y_pred_no_SA)
    ...
    ret_true.append(capital_evolution_true)
    print('capital_evolution_true shape', capital_evolution_true.shape)
    ret_no_SA.append(capital_evolution_no_SA)
    print('capital_evolution_no_SA shape', capital_evolution_no_SA.shape)
    
    total_returns_true.append(total_return_true)
    total_returns_no_SA.append(total_return_no_SA)
...
# -------------------------------------------------------------------------------------------------------------------
    ...
    all_mse_values_finbert.append(history_finbert.history['loss'])  # Append MSE values for the current fold
    
    # mse_values.append(mse)
    predicted_prices_finbert.append(y_pred_finbert)
    ...
    ret_finbert.append(capital_evolution_finbert)
    print('capital_evolution_finbert shape', capital_evolution_finbert.shape)
    
    total_returns_finbert.append(total_return_finbert)
...
# -------------------------------------------------------------------------------------------------------------------
    ...
    all_mse_values_distilbert.append(history_distilbert.history['loss'])  # Append MSE values for the current fold
    
    # mse_values.append(mse)
    predicted_prices_distilbert.append(y_pred_distilbert)
        
    # Capital evolution over time and Total Returns
    
    total_return_distilbert, capital_evolution_distilbert = calculate_cumproduct_strategy(y_test, y_pred_distilbert)
    
    ret_distilbert.append(capital_evolution_distilbert)
    print('capital_evolution_distilbert shape', capital_evolution_distilbert.shape)
    
    total_returns_distilbert.append(total_return_distilbert)
...
# -------------------------------------------------------------------------------------------------------------------
    ...
    all_mse_values_CNN.append(history_CNN.history['loss'])  # Append MSE values for the current fold
    
    # mse_values.append(mse)
    predicted_prices_CNN.append(y_pred_CNN)
    ...
    ret_CNN.append(capital_evolution_CNN)
    print('capital_evolution_CNN shape', capital_evolution_CNN.shape)
    
    total_returns_CNN.append(total_return_CNN)
...
plt.plot([sum(col) / len(col) for col in zip(*true_prices)], label='True Prices', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*predicted_prices_no_SA)], label='Predicted Prices avg no SA', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*predicted_prices_finbert)], label='Predicted Prices avg finbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*predicted_prices_distilbert)], label='Predicted Prices avg distilbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*predicted_prices_CNN)], label='Predicted Prices avg CNN', alpha=0.5)
plt.legend()
plt.title(f'True vs. Predicted Prices avg among all folds')
plt.xlabel('Time')
plt.ylabel('Price')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/K_Fold_splitting/True vs. Predicted Prices avg among all folds.png')
plt.show()

# Plot Mean Squared Error for the average of all folds over the number of epochs
plt.figure(figsize=(12, 6))
plt.plot([sum(col) / len(col) for col in zip(*all_mse_values_no_SA)], label='mse_values_avg_no_SA', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*all_mse_values_finbert)], label='mse_values_avg_finbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*all_mse_values_distilbert)], label='mse_values_avg_distilbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*all_mse_values_CNN)], label='mse_values_avg_CNN', alpha=0.5)
plt.legend()
plt.title(f'MSE Over Epochs avg all Folds')
plt.xlabel('Epochs')
plt.ylabel('MSE')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/K_Fold_splitting/MSE Over Epochs avg all Folds.png')
plt.show()
...
plt.figure(figsize=(12, 6))
plt.plot([sum(col) / len(col) for col in zip(*ret_true)], label='Long only', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*ret_no_SA)], label='Avg LSTM no SA', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*ret_finbert)], label='Avg LSTM finbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*ret_distilbert)], label='Avg LSTM distilbert', alpha=0.5)
plt.plot([sum(col) / len(col) for col in zip(*ret_CNN)], label='Avg LSTM CNN', alpha=0.5)
plt.legend()
plt.title(f'Evolution of 1000$ invested with the following strategies avg all Folds')
plt.xlabel('Time')
plt.ylabel('Money')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/K_Fold_splitting/evolution_1000$_avg_all_folds.png')
plt.show()
...
confronto['Avg perf'] = [sum(total_returns_true) / num_folds, 
                          sum(total_returns_no_SA) / num_folds, 
                          sum(total_returns_finbert) / num_folds, 
                          sum(total_returns_distilbert) / num_folds, 
                          sum(total_returns_CNN) / num_folds]

confronto.to_csv(parent_directory + '/Output/LSTM/with_memory/K_Fold_splitting/confronto_avg_perf.csv')
...
styled_confronto.to_html(parent_directory + '/Output/LSTM/with_memory/K_Fold_splitting/styled_confronto_total_returns_avg.html')
