...
# Specifically this code will consider a time-based dataset splitting with past Close prices included in the feature set.
# ---------------------------------------
...
def LSTM_df_builder(df, time_lag_max):
    ...
    for name in column_names:
        for i in range(2, time_lag_max + 1): # from 2 because I already shifted by 1
            if name != 'Close': # Close price wasn't subject to shifting 
            ...
            else:  # Correct indentation for the 'Close' condition
                new_colname = f'{name} t-{i-1}'
                df[new_colname] = np.nan
                plc_hld = []
                for row in range(i - 1, df.shape[0]):
                    if row - i + 1 >= 0:  # Check if the index is non-negative
...
# Hyperparameter for deciding the forecasted time frequency (in this case daily) 
num_time_steps = 1
...
# Replace 'your_dataset.csv' with the actual filename or data loading process
...
# Add the Close price back to keep information about past closing pricess
features_no_SA['Close'] = data['Close']
features_distilbert['Close'] = data['Close']
features_finbert['Close'] = data['Close']
features_CNN['Close'] = data['Close']
...
# Pull out the Close price because it is the Target variable
features_no_SA = features_no_SA.drop(columns=['Close'])
features_distilbert = features_distilbert.drop(columns=['Close'])
features_finbert = features_finbert.drop(columns=['Close'])
features_CNN = features_CNN.drop(columns=['Close'])
...
# FOR DEBUGGING:
features_finbert.to_csv('features_finbert.csv')
...
# y_test_reshaped = y_test.reshape(-1, num_time_steps, 1) # Single Target Prediction Task -> Close
print('Shape y_train_reshaped:',y_train_reshaped.shape)
# print('Shape y_test_reshaped:',y_test_reshaped.shape)
...
# y_test_tensor = tf.constant(y_test_reshaped, dtype=tf.float32)
print('Shape y_train_tensor:',y_train_tensor.shape)
# print('Shape y_test_tensor:',y_test_tensor.shape)
...
# true_prices.append(y_test_tensor.numpy())  # Convert to a NumPy array
true_prices = y_test
...
plt.ylabel('Price')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/time_based_splitting/with_past_Close_prices/True vs. Predicted Prices time-based split.png')
plt.show()
...
plt.ylabel('MSE')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/time_based_splitting/with_past_Close_prices/MSE Over Epochs time-based split.png')
plt.show()
...
plt.ylabel('Money')
plt.grid(True)
plt.savefig(parent_directory + '/Output/LSTM/with_memory/time_based_splitting/with_past_Close_prices/evolution_1000$_time-based_split.png')
plt.show()
...
confronto.to_csv(parent_directory + '/Output/LSTM/with_memory/time_based_splitting/with_past_Close_prices/confronto time-based split.csv')
...
styled_confronto.to_html(parent_directory + '/Output/LSTM/with_memory/time_based_splitting/with_past_Close_prices/styled_confronto_total_returns_time-based_plit.html')
